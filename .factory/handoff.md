...
Deployed to `https://mobile-sprite-pack.sociobot.in/` from repair commit `e6a5085` on 2026-08-27 UTC. Live checks confirmed the new `index-DoEH_l5p.js` bundle, `Cache-Control: no-cache` for `/`, `/sw.js`, and the non-hashed hero; `public, max-age=31536000, immutable` for the hashed JS; and CSP, Permissions-Policy, and `X-Frame-Options: DENY`. The factory `verify-url.sh` check returned HTTP 200 in 1112ms with no console errors, title/lang, one `h1`, a `main` landmark, and zero images missing alt text.
