...
const API=location.hostname==='mobile-sprite-pack.sociobot.in'?'https://api.sociobot.in/api/v1':'https://pilot-api.sociobot.in/api/v1';
type Verdict={ valid:boolean; checkedAt:number; reason?:string };

export function captureLicenseFromUrl(){ const url=new URL(location.href); const token=url.searchParams.get('license'); if(token){localStorage.setItem(TOKEN_KEY,token);localStorage.removeItem(VERDICT_KEY);url.searchParams.delete('license');history.replaceState({},'',url.pathname+url.search+url.hash);} return token; }
export function checkoutUrl(){return `${API}/products/${SLUG}/checkout`;}
